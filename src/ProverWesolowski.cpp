...
ProverWesolowski::ProverWesolowski() : durations(4), ctx_ptr(BN_CTX_free_ptr(BN_CTX_secure_new(), ::BN_CTX_free)) {}

typename ProverWesolowski::solution ProverWesolowski::operator()(const VerifierWesolowski &verifier) const {
  return operator()(verifier, false);
}

typename ProverWesolowski::solution ProverWesolowski::operator()(const VerifierWesolowski &verifier,
